...
export default function PortalAlumno() {
  ...
  const [ejercicioSeleccionadoInfo, setEjercicioSeleccionadoInfo] =
    useState(null);
  ...
  const handleSeleccionarIntegrante = (integrante) => {
    ...
    if (clienteReal && clienteReal.id) {
      ...
      console.error(
        "ALERTA: Tu backend solo envió el nombre del cliente, pero NO su ID. No se pueden buscar sus rutinas.",
      );
      setRutinasAlumno([]);
    }
    ...
    <div className="min-h-screen bg-[#F8FAFC] font-sans flex flex-col relative">
            ...
            <div className="p-6 space-y-6">
              <div>
                ...
                <p className="text-slate-700 leading-relaxed font-medium">
                  {ejercicioSeleccionadoInfo.descripcion ||
                    "No hay descripción detallada para este ejercicio aún."}
                </p>
              </div>
      ...
      <header className="bg-slate-900 text-white p-4 shadow-md flex items-center justify-center relative z-10 sticky top-0">
        {paso !== "inicio" && (
          <button
            onClick={reiniciar}
            className="absolute left-4 p-2 text-slate-300 hover:text-white transition-colors"
          >
            <svg
              className="w-6 h-6"
              fill="none"
      ...
      {/* MAIN */}
      <main
        className={`flex-1 max-w-md w-full mx-auto p-6 flex flex-col ${paso === "rutina" ? "justify-start" : "justify-center"}`}
      >
        {paso === "inicio" && (
          <div className="space-y-6 animate-fade-in">
            <div className="text-center mb-8">
              ...
              <p className="text-slate-500 text-sm mt-2">
                Selecciona tu modalidad de entrenamiento para ver tu rutina de
                hoy.
              </p>
            </div>
            <button
              onClick={() => handleSeleccionTipo("individual")}
              className="w-full bg-white border-2 border-emerald-100 hover:border-emerald-500 rounded-2xl p-6 text-left flex items-center gap-4 transition-all shadow-sm active:scale-95 group"
            >
              ...
              <div>
                ...
                <p className="text-xs text-slate-500">
                  Ingreso para miembros únicos
                </p>
              </div>
            </button>
            <button
              onClick={() => handleSeleccionTipo("familia")}
              className="w-full bg-white border-2 border-emerald-100 hover:border-emerald-500 rounded-2xl p-6 text-left flex items-center gap-4 transition-all shadow-sm active:scale-95 group"
            >
              <div className="w-14 h-14 bg-emerald-50 rounded-full flex items-center justify-center group-hover:bg-emerald-500 transition-colors">
                <svg
              ...
              <div>
                ...
                <p className="text-xs text-slate-500">
                  Ingresa al grupo de tu familia
                </p>
              </div>
            </button>
          </div>
        ...
        {paso === "buscar" && (
          <div className="animate-fade-in w-full">
            <h2 className="text-xl font-bold text-slate-800 mb-6 text-center">
              ¿Cuál es tu{" "}
              {tipoIngreso === "familia" ? "grupo familiar" : "nombre"}?
            </h2>
            <input
              type="text"
              placeholder={`Buscar ${tipoIngreso === "familia" ? "familia..." : "nombre..."}`}
              className="w-full bg-white border border-slate-200 rounded-xl px-4 py-3 mb-6 focus:outline-none focus:ring-2 focus:ring-emerald-500 text-slate-800 shadow-sm"
              value={busqueda}
              ...
              <div className="bg-white rounded-xl shadow-sm border border-slate-100 divide-y divide-slate-100 overflow-hidden max-h-80 overflow-y-auto">
                {listaFiltrada.length === 0 ? (
                  ...
                  listaFiltrada.map((item) => (
                    <button
                      key={item.id}
                      onClick={() => handleSeleccionarPerfil(item)}
                      className="w-full p-4 text-left font-medium text-slate-700 hover:bg-emerald-50 transition-colors"
                    >
                      {item.nombre}
                    </button>
                  ))
        ...
        {paso === "pin" && (
          <div className="animate-fade-in w-full text-center">
            ...
            <p className="text-sm text-slate-500 mb-8">
              Ingresa los últimos 4 dígitos del RUT (sin dígito verificador)
            </p>
            <form onSubmit={handleVerificarPin}>
              <input
                ...
                maxLength="4"
                pattern="\d*"
                className={`w-32 mx-auto text-center text-3xl font-bold tracking-[0.5em] bg-white border-2 rounded-xl px-4 py-3 mb-4 focus:outline-none focus:ring-2 ${errorPin ? "border-red-500 focus:ring-red-200" : "border-slate-200 focus:border-emerald-500 focus:ring-emerald-200"} shadow-sm`}
                value={pin}
              ...
              <button
                ...
                className="w-full mt-6 bg-slate-900 text-white font-bold py-3.5 rounded-xl hover:bg-emerald-600 transition-all shadow-md active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed"
              >
        ...
        {paso === "familia" && seleccionado && (
          <div className="animate-fade-in w-full">
            ...
            <div className="space-y-3">
              {seleccionado.integrantes &&
                seleccionado.integrantes.map((integrante, index) => {
                  const nombreMostrar =
                    obtenerNombreReal(integrante) ||
                    `Familiar sin nombre (${index + 1})`;
          ...
          <div className="animate-fade-in w-full pb-8">
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                <h2 className="text-xl font-bold text-slate-900">
                  Hola,{" "}
                  {integranteActivo?.nombre
                    ? integranteActivo.nombre.split(" ")[0]
                    : "Alumno"}
                </h2>
            ...
            {cargandoRutinas ? (
              ...
              <div className="bg-white border-2 border-dashed border-slate-200 rounded-2xl p-8 text-center">
                ...
                <p className="text-slate-500 text-sm">
                  No tienes rutinas asignadas en este momento. Consulta con tu
                  entrenador.
                </p>
              </div>
            ) : (
              <div className="space-y-6">
                    ...
                    >
                      ...
                      >
                        <div className="flex items-center gap-3">
                          <h3 className="font-bold text-white text-lg text-left">
                            {rutina.nombre}
                          </h3>
                      ...
                      {isExpanded && (
                        <div className="divide-y divide-slate-100 p-2 animate-fade-in">
                          {rutina.ejercicios.map((ej) => (
                            <div
                              key={ej.id}
                              className="flex items-start gap-4 p-4 hover:bg-slate-50 transition-colors rounded-xl"
                            >
                              <div className="mt-1">
                                <input
                                  type="checkbox"
                                  className="w-6 h-6 text-emerald-500 border-gray-300 rounded-md focus:ring-emerald-500 cursor-pointer"
                                />
                              </div>
                ...
                <button
                  onClick={reiniciar}
                  className="w-full bg-emerald-50 text-emerald-700 font-bold py-4 rounded-2xl hover:bg-emerald-100 transition-colors border border-emerald-200"
                >
